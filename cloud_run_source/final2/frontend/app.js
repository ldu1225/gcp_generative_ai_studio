document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('.btn-enhance').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            try {
                ...
                // Animation is 0.6s, remove class after it finishes
                setTimeout(() => {
    ...
    upscaleBtnStandalone.addEventListener('click', async () => {
        ...
        try {
            const response = await fetch('/api/upscale_image', {
                method: 'POST',
                body: formData
            });
            if (!response.ok) throw new Error(`Server error: ${response.statusText}`);
    ...
    videoForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (!response.ok) {
                // Use the error message from the server, which might include quota info
    ...
    campaignGenerateBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            // Use imageData (base64) to prevent canvas CORS issues.
    ...
    finetuneBtn.addEventListener('click', async () => {
        ...
        try {
            const response = await fetch('/api/simulate_finetuning', {
                method: 'POST',
                body: formData
            });
            if (!response.ok) throw new Error(`Server error: ${response.statusText}`);
