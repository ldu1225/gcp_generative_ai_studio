...
def upload():
    ...
    coordinates = request.form.get('coordinates', '0,0')
    ...
    # Use the global GCS client and upload_to_gcs helper
    image_url, _ = upload_to_gcs(storage_client, BUCKET_NAME, img_byte_arr, 'image/png', 'edited_nano')
    ...
    try:
        ...
        while True:
            ...
            if op_status.get("done"):
                ...
                if not video_list:
                    raise ValueError("Prediction result is empty ('videos' key not found or empty). Check logs for full API response.")
