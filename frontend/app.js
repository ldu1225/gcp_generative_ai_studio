document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- Image Edit Tab (Nano-banana) ---
    const editFormNano = document.getElementById('edit-form-nano');
    if (editFormNano) {
        const nanoUploadInput = document.getElementById('image-edit-upload-nano');
        const nanoPreviewGallery = document.getElementById('nano-preview-gallery');
        let nanoFiles = [];

        nanoUploadInput.addEventListener('change', (event) => {
            const newFiles = Array.from(event.target.files);
            if (nanoFiles.length + newFiles.length > 3) {
                alert('You can upload a maximum of 3 images.');
                // Reset the file input to allow re-selecting the same file if needed
                event.target.value = '';
                return;
            }
            nanoFiles.push(...newFiles);
            renderNanoPreviews();
            // Reset the file input
            event.target.value = '';
        });

        function renderNanoPreviews() {
            nanoPreviewGallery.innerHTML = '';
            nanoFiles.forEach((file, index) => {
                const reader = new FileReader();
                reader.onload = (e) => {
                    const previewWrapper = document.createElement('div');
                    previewWrapper.classList.add('nano-preview-item');
                    
                    const img = document.createElement('img');
                    img.src = e.target.result;
                    
                    const removeBtn = document.createElement('button');
                    removeBtn.classList.add('nano-remove-btn');
                    removeBtn.innerHTML = '&times;';
                    removeBtn.onclick = () => {
                        nanoFiles.splice(index, 1);
                        renderNanoPreviews();
                    };
                    
                    previewWrapper.appendChild(img);
                    previewWrapper.appendChild(removeBtn);
                    nanoPreviewGallery.appendChild(previewWrapper);
                };
                reader.readAsDataURL(file);
            });
        }

        editFormNano.addEventListener('submit', async (e) => {
            e.preventDefault();
            if (nanoFiles.length === 0) {
                alert('Please upload at least one image.');
                return;
            }

            const loading = document.getElementById('image-edit-loading-nano');
            const editResults = document.getElementById('image-edit-results-nano');
            loading.classList.remove('hidden');
            editResults.classList.add('hidden');

            const formData = new FormData();
            formData.append('prompt', document.getElementById('image-edit-prompt-nano').value);
            nanoFiles.forEach(file => {
                formData.append('images', file);
            });

            try {
                const response = await fetch('/api/edit_nanobanana', { method: 'POST', body: formData });
                const result = await handleApiError(response);
                document.getElementById('edited-image-nano').src = result.editedImageUrl;
                editResults.classList.remove('hidden');
            } catch (error) {
                alert('Error editing image with Nano-banana: ' + error.message);
            } finally {
                loading.classList.add('hidden');
            }
        });
