...
def init_clients():
    global storage_client, gemini_planner, edit_model, upscale_model, generate_model, video_model, vision_client, tts_client, prediction_client
    try:
        vertexai.init(project=PROJECT_ID, location=LOCATION)
        storage_client = storage.Client(project=PROJECT_ID)
        gemini_planner = GenerativeModel(GEMINI_PLANNER_MODEL)
        ...
        video_model = GenerativeModel(VEO_MODEL_ID)
        ...
        log.info("All AI models and GCS clients initialized successfully using Vertex AI SDK.")
    except Exception as e:
        log.critical(f"Failed to initialize Google Cloud services: {e}", exc_info=True)
        raise
...
def call_gemini_for_editing_plan(gcs_uri: str, user_prompt: str) -> dict:
    ...
    org_image = Part.from_uri(gcs_uri, mime_type="image/png")
    prompt_template = f"""
    ...
    if edit_type == "EDIT_MODE_DEFAULT":
        ...
        if edit_mode == "default":
            ...
            if mask_mode == "MASK_MODE_FOREGROUND":
                mask_ref_image = MaskReferenceImage(reference_id=1, image=None, mask_mode="foreground", dilation=mask_dilation) 
            elif mask_mode == "MASK_MODE_BACKGROUND":
                mask_ref_image = MaskReferenceImage(reference_id=1, image=None, mask_mode="background", dilation=mask_dilation)
            else:
                mask_ref_image = MaskReferenceImage(reference_id=1, image=None, mask_mode="default", dilation=mask_dilation)
...
def simulate_finetuning_api():
    ...
    try:
        ...
        for file in files:
            ...
            style_image_parts.append(Part.from_uri(gcs_uri, mime_type=file.mimetype))
    ...
    try:
        ...
        while True:
            ...
            if op_status.get("done"):
                ...
                with open("backend/video_generation_response.json", "w") as f:
                    json.dump(op_status, f, indent=2)
                log.info("Full response saved to backend/video_generation_response.json")

                if "error" in op_status:
                    error_details = op_status["error"]
                    log.error(f"LRO finished with error: {error_details}")
                    raise Exception(f"Video generation failed: {error_details.get('message')}")
                ...
                if not video_list:
                    raise ValueError("Prediction result is empty ('videos' key not found or empty). Check video_generation_response.json for details.")
                
                # Extract video data from the correct path
                video_b64 = video_list[0].get("bytesBase64Encoded")
                if not video_b64:
                    raise ValueError("'bytesBase64Encoded' key not found in the video response.")
        ...
        with open("backend/video_generation_error.log", "w") as f:
...
def analyze_image_api():
    ...
    try:
        # --- Vision API Analysis (Still used for object boxes and initial safety check) ---
        ...
        image_part = Part.from_uri(gcs_uri, mime_type="image/png")
        ...
        # --- HSAD Policy Check (Now with Gemini's input) ---
        ...
        # Check Gemini labels for policy violations
